...
class Label(fantas.Ui):
	...
	__slots__ = ('bd', 'bg', 'sc', 'layout_data', 'radius')

	def __init__(self, size, bd=0, bg=None, sc=None, radius={}, **anchor):
		...
		if self.bg is not None:
			pygame.draw.rect(self.img, self.bg, self.img.get_rect(), **self.radius)
		if self.bd > 0 and self.sc is not None:
			pygame.draw.rect(self.img, self.sc, (0,0,*self.origin_size), width=round(self.bd), **self.radius)
		self.angle = 0
	...
	def set_radius(self, r):
		self.radius['border_radius'] = round(r)
		self.mark_update()
...
class CircleLabel(Label):
	...
	def update_img(self):
		self.img = pygame.Surface((self.radius*2,self.radius*2), flags=pygame.SRCALPHA)
		if self.bg is not None and self.radius > 0:
			pygame.draw.circle(self.img, self.bg, (self.radius,self.radius), self.radius)
		if self.bd > 0 and self.sc is not None:
			pygame.draw.circle(self.img, self.sc, (self.radius,self.radius), self.radius, width=round(self.bd))
		self.layout()
